...
onMounted(async () => {
  console.log('XTerm mounting for session:', props.sessionId)
  ...
  // 创建后端PTY会话
  try {
    const result = await invoke('create_pty_session', {
    ...
    console.log('PTY session created successfully:', result)
  } catch (error) {
...
watch(() => terminalContainer.value, (newVal) => {
  if (newVal && fitAddon) {
    // 当组件从隐藏变为显示时，重新 fit
    setTimeout(() => {
      fitAddon.fit()
