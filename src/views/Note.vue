...
<template>
  ...
  >
    ...
    <!-- 笔记列表 -->
    <div class="flex-1 overflow-y-auto p-3 min-h-0" style="scrollbar-width: thin; scrollbar-color: #3a3a45 transparent;">
      <a-empty
        v-if="notes.length === 0"
        :image="false"
        description=""
    ...
    <!-- 拖拽遮罩 -->
    <div :class="['fixed inset-0 bg-[rgba(0,255,136,0.05)] border-2 border-dashed border-[#00ff88] flex flex-col items-center justify-center z-[100] opacity-0 pointer-events-none transition-opacity duration-200', { 'opacity-100 pointer-events-auto': isDragging }]">
    ...
    >
      ...
      <div v-if="drawerImages.length > 0" class="flex gap-3 mb-4 flex-wrap">
        <img
          v-for="(img, index) in drawerImages"
          :key="index"
          :src="img"
          class="w-full max-w-[180px] rounded-lg object-cover border border-[#2a2a32]"
          alt="Note image"
        />
      </div>
      ...
      <div v-if="drawerSourceUrl" class="mt-4 pt-4 border-t border-[#2a2a32] text-xs text-[#4a4a55]">
        来源: <a :href="drawerSourceUrl" target="_blank" class="text-[#60a5fa] hover:underline">{{ drawerSourceUrl }}</a>
      </div>
    </a-drawer>
  </div>
</template>