...
pub fn create_todo(
    ...
    let reminder_json = reminder.unwrap_or_else(|| {
        serde_json::to_string(&TodoReminder {
            ...
            match reminder.reminder_type {
                ReminderType::OneTime => {
                    if let Some(ref repeat_time) = reminder.repeat_time {
                        if let Ok(reminder_time) =
                            chrono::NaiveDateTime::parse_from_str(repeat_time, "%Y-%m-%dT%H:%M")
                        {
                            // 转换为本地时间进行比较

                            let reminder_local =
                                reminder_time.and_local_timezone(chrono::Local).unwrap();
